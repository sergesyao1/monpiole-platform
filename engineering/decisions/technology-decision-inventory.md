...
| TD-009 | CI execution | Execute deterministic local checks in automation, define triggers, failure semantics, diagnostics, reports, and governed exceptions. | ADR-0002; ADR-0006; [approved TD-009](td-009-ci-execution-technology-proposal.md); TASK-006-08; TASK-021; `scripts/ci/README.md` | **APPROVED — NOT IMPLEMENTED.** GitHub Actions with ephemeral GitHub-hosted Ubuntu 24.04 x64 runners is selected to orchestrate all governed TD-001 through TD-008 gates. The pre-existing architecture-only workflow remains unchanged limited scope and is not full CI coverage. | Implemented local gates through TD-008; current GitHub hosting; Docker/Testcontainers and immutable PostgreSQL 18 image policy. A separately authorized TASK-022 is required and has not started. | Implement only through separately governed TASK-022. Keep workflows thin, local commands authoritative, permissions read-only, Testcontainers exclusive for PostgreSQL, immutable references enforced, and production/CD concerns deferred. | 6, decision approved; expanded implementation not started. |
