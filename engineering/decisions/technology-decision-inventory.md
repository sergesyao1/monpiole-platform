...
- Status: Governed inventory; TD-006 approved technology direction not implemented
- Date: 2026-08-25
...
| TD-006 | API implementation and contract representation | Implement versioned APIs, authentication/authorisation boundaries, validation, errors, tenant/correlation propagation, lifecycle, and compatibility verification. ADR-0003 defines policy but not implementation technology or schema representation. | ADR-0003; ADR-0004; ADR-0005; AGENTS.md API-first rules; [approved TD-006](td-006-api-contract-representation-proposal.md); TASK-016 | **APPROVED — NOT IMPLEMENTED.** Zod 4 is the canonical executable transport-contract schema representation; `nestjs-zod` remains in the NestJS interface/composition boundary; `@nestjs/swagger` assembles/publishes OpenAPI 3.1; RFC 9457, URI major versioning, explicit context/idempotency, boundary separation, and Vitest contract verification conventions are approved. | Implementation requires a separate governed task plus exact compatibility, supply-chain, license, provenance, and lockfile verification. Persistence, messaging, authentication-provider, telemetry, and deployment selections remain deferred. | Use the approved TD-006 direction for the later implementation task; approval does not authorize dependency installation or API implementation. | 4, technology selection complete; implementation separately gated. |
