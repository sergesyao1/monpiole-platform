...
- Status: Governed inventory; TD-009 CI execution baseline implemented by TASK-022
- Date: 2026-08-25
...
| TD-009 | CI execution | Execute deterministic local checks in automation, define triggers, failure semantics, diagnostics, reports, and governed exceptions. | ADR-0002; ADR-0006; [approved TD-009](td-009-ci-execution-technology-proposal.md); TASK-006-08; TASK-021; TASK-022; `scripts/ci/README.md` | **APPROVED — BASELINE IMPLEMENTED BY TASK-022.** GitHub Actions uses ephemeral GitHub-hosted Ubuntu 24.04 x64 runners to execute all governed TD-001 through TD-008 gates once where possible, then publishes the stable `CI / required` aggregate result. The historical architecture-only workflow path now contains the expanded graph; architecture remains enforced once. | Implemented local gates through TD-008; current GitHub hosting; Docker/Testcontainers and immutable PostgreSQL 18 image policy. Repository-owner ruleset configuration and hosted-runner acceptance evidence remain operational follow-up actions. | Preserve thin workflows, local commands as authoritative, read-only permissions, Testcontainers-exclusive PostgreSQL, immutable references, and the stable aggregate name. Production/CD concerns remain deferred. | 6, technical CI baseline implemented; repository ruleset activation remains owner-governed. |
...
| TD-013 | Web frontend foundation | Establish the browser application framework, build, routing and UI test baseline for `apps/web`. | UI-001; ADR-0001; ADR-0002; ADR-0007 | **APPROVED — BASELINE IMPLEMENTED BY UI-001.** React 19, Vite 8, React Router 8 declarative mode, native CSS and Vitest/Testing Library are exact-pinned for the initial SPA shell. | TD-001, TD-002, TD-004 and the public API/OIDC boundaries. | [Approved TD-013](td-013-web-frontend-foundation.md). OIDC client integration and deployment remain separate decisions/tasks. | Implemented as the first web foundation. |
| TD-014 | Web managed OIDC client | Integrate the Auth0 SPA lifecycle without coupling features to the provider or moving business authority into the browser. | UI-002; ADR-0007; TD-013 | **APPROVED — BASELINE IMPLEMENTED BY UI-002.** Official Auth0 React SDK, in-memory cache, provider-neutral session boundary and one controlled token refresh after 401. | Auth0 environment provisioning and API issuer/audience alignment. | [Approved TD-014](td-014-web-managed-oidc-client.md). | Implemented as the Web authentication/session boundary. |
| TD-015 | Web OIDC tab-session cache | Make Auth0 session restoration reliable across a full reload without durable, cross-tab localStorage persistence. | UI-002A; ADR-0007; TD-014 | **APPROVED — IMPLEMENTED BY UI-002A.** SDK `ICache` backed by namespaced sessionStorage; rotating refresh token remains SDK-managed. | XSS can access tokens during the tab session; CSP and frontend hardening remain material controls. | [Approved TD-015](td-015-web-oidc-tab-session-cache.md). | Supersedes only TD-014's memory-cache choice. |
