...
export const propertyApplications=propertyManagement.table("property_applications",{applicationId:uuid("application_id").primaryKey(),tenantId:uuid("tenant_id").notNull(),propertyId:uuid("property_id").notNull(),inquiryId:uuid("inquiry_id").notNull(),viewingId:uuid("viewing_id").notNull(),outcomeId:uuid("outcome_id").notNull(),status:text("status").notNull(),note:text("note"),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull(),updatedAt:timestamp("updated_at",{withTimezone:true,mode:"string"}).notNull(),decidedAt:timestamp("decided_at",{withTimezone:true,mode:"string"}),correlationId:uuid("correlation_id").notNull(),actorId:text("actor_id").notNull()},table=>[
 uniqueIndex("property_applications_tenant_application_unique").on(table.tenantId,table.applicationId),uniqueIndex("property_applications_tenant_viewing_unique").on(table.tenantId,table.viewingId),uniqueIndex("property_applications_tenant_outcome_unique").on(table.tenantId,table.outcomeId),foreignKey({name:"property_applications_property_tenant_fk",columns:[table.tenantId,table.propertyId],foreignColumns:[properties.tenantId,properties.propertyId]}),foreignKey({name:"property_applications_viewing_tenant_fk",columns:[table.tenantId,table.propertyId,table.viewingId],foreignColumns:[propertyViewings.tenantId,propertyViewings.propertyId,propertyViewings.viewingId]}),foreignKey({name:"property_applications_outcome_tenant_fk",columns:[table.tenantId,table.outcomeId],foreignColumns:[propertyViewingOutcomes.tenantId,propertyViewingOutcomes.outcomeId]}),foreignKey({name:"property_applications_inquiry_tenant_fk",columns:[table.tenantId,table.inquiryId],foreignColumns:[propertyInquiries.tenantId,propertyInquiries.inquiryId]}),index("property_applications_tenant_property_created_idx").on(table.tenantId,table.propertyId,table.createdAt.desc(),table.applicationId.desc()),check("property_applications_note_check",sql`${table.note} IS NULL OR char_length(btrim(${table.note})) BETWEEN 1 AND 2000`),check("property_applications_lifecycle_check",sql`(${table.status}='SUBMITTED' AND ${table.decidedAt} IS NULL) OR (${table.status} IN ('APPROVED','REJECTED','WITHDRAWN') AND ${table.decidedAt} IS NOT NULL)`),pgPolicy("property_applications_tenant_isolation",{using:sql`${table.tenantId}=NULLIF(current_setting('app.tenant_id',true),'')::uuid`,withCheck:sql`${table.tenantId}=NULLIF(current_setting('app.tenant_id',true),'')::uuid`})]).enableRLS();
