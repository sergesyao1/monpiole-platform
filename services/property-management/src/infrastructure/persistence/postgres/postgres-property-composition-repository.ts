...
export class PostgresPropertyCompositionRepository implements PropertyCompositionRepository {
  ...
  createUnit(tenantId: string, propertyId: string, buildingId: string, unitCode: string, unit: Property, trace: Trace): Promise<PropertyUnitView | undefined> {
    return this.conflicts(() => withTenantPostgresTransaction(this.pool, tenantId, async (scope) => {
      ...
      await scope.database().insert(propertyBuildingUnits).values({ tenantId, buildingId, unitPropertyId: value.propertyId, unitCode, createdAt: value.createdAt, updatedAt: value.updatedAt, correlationId: trace.correlationId, actorId: trace.actorId });
  ...
  listUnits(tenantId: string, propertyId: string, buildingId: string, limit: number, cursor?: CompositionCursor): Promise<CompositionPage<PropertyUnitView> | undefined> {
    return withTenantPostgresTransaction(this.pool, tenantId, async (scope) => {
      ...
      const items = rows.slice(0, limit).map((row) => ({ unitCode: row.relation.unitCode, property: toProperty(row.property).values })); const last = rows.length > limit ? items.at(-1) : undefined;
  ...
  updateUnitCode(tenantId: string, propertyId: string, buildingId: string, unitPropertyId: string, unitCode: string, updatedAt: string, trace: Trace): Promise<PropertyUnitView | undefined> {
    return this.conflicts(() => withTenantPostgresTransaction(this.pool, tenantId, async (scope) => {
      ...
      const relation = (await scope.database().select().from(propertyBuildingUnits).where(and(eq(propertyBuildingUnits.tenantId, tenantId), eq(propertyBuildingUnits.buildingId, buildingId), eq(propertyBuildingUnits.unitPropertyId, unitPropertyId))).for("update").limit(1))[0]; if (relation === undefined) return undefined;
      await scope.database().update(propertyBuildingUnits).set({ unitCode, updatedAt, correlationId: trace.correlationId, actorId: trace.actorId }).where(and(eq(propertyBuildingUnits.tenantId, tenantId), eq(propertyBuildingUnits.buildingId, buildingId), eq(propertyBuildingUnits.unitPropertyId, unitPropertyId)));
      ...
      return { unitCode, property: toProperty(property).values };
    }), "unit");
  }

  private async conflicts<T>(operation: () => Promise<T>, kind: "building" | "unit"): Promise<T> {
    try { return await operation(); } catch (error) {
      if (isUniqueViolation(error)) throw kind === "building" ? new PropertyBuildingCodeConflictError() : new PropertyUnitCodeConflictError();
      throw error;
    }
  }
}
