import { PROPERTY_TYPES, type PropertyStatus, type PropertyType } from "../domain/property.js";
...
export class ListProperties {
  ...
  async execute(query: ListPropertiesQuery): Promise<PropertyPortfolioPage> {
    ...
    if (query.status !== undefined && query.status !== "DRAFT") throw new InvalidPropertyPortfolioQueryError("status");
