...
export class PropertyCompositionController {
  ...
  async postUnit(@Param() path: PropertyCompositionBuildingPathDto, @Body() body: CreateUnitDto, @Req() request: RequestWithContext) { const unit = await this.createUnit.execute({ ...await this.context(request), propertyId: path.propertyId, buildingId: path.buildingId, ...body }); return { unitCode: unit.unitCode, property: toPropertyResponse(unit.property) }; }
  ...
  async getUnits(@Param() path: PropertyCompositionBuildingPathDto, @Query() query: CompositionQueryDto, @Req() request: RequestWithContext) { const page = await this.listUnits.execute({ ...await this.context(request), propertyId: path.propertyId, buildingId: path.buildingId, limit: query.limit, ...(query.cursor ? { cursor: decodeCompositionCursor(query.cursor) } : {}) }); return pageResponse({ ...page, items: page.items.map((item) => ({ unitCode: item.unitCode, property: toPropertyResponse(item.property) })) }); }
  ...
  async putUnit(@Param() path: PropertyCompositionUnitPathDto, @Body() body: UnitMutationDto, @Req() request: RequestWithContext) { const unit = await this.updateUnit.execute({ ...await this.context(request), propertyId: path.propertyId, buildingId: path.buildingId, unitPropertyId: path.unitPropertyId, unitCode: body.unitCode }); return { unitCode: unit.unitCode, property: toPropertyResponse(unit.property) }; }
}
