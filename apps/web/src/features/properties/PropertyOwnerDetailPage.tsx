...
import { Alert, LoadingState, PageHeader, StatusBadge, buttonClassName } from "../../ui/index.js";
...
export function PropertyOwnerDetailPage() {
  ...
  if (loading) return <div className="standalone-state"><LoadingState label="Chargement du propriétaire…" /></div>;
  if (!owner) return <div className="standalone-state"><h1>Propriétaire introuvable</h1><PropertyFeedback error={error ?? toPropertyUiError(undefined)} onReconnect={() => void session.login(`/proprietaires/${ownerId}`)} /><Link className={buttonClassName("secondary", "inline-action")} to="/proprietaires">Retour à l’annuaire</Link></div>;
  return <div className="page-stack"><PageHeader actions={<Link className={buttonClassName("secondary", "inline-action")} to="/proprietaires">Retour à l’annuaire</Link>} breadcrumbs={[{ label: "Propriétaires", to: "/proprietaires" }, { label: propertyOwnerName(owner) }]} eyebrow="Fiche propriétaire" title={propertyOwnerName(owner)} meta={<><StatusBadge tone="info">{owner.ownerType === "INDIVIDUAL" ? "Personne physique" : "Personne morale"}</StatusBadge><span className="resource-id">Référence interne : {owner.ownerId}</span></>} />{saved && <Alert tone="success" title="Enregistré"><p>Les informations du propriétaire sont à jour.</p></Alert>}{error && <PropertyFeedback error={error} />}<section className="content-panel"><PropertyOwnerForm owner={owner} saving={saving} onSubmit={save} /></section></div>;
}
