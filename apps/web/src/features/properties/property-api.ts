...
import type {
  ...
  Amenity,
  PropertyInquiry, PropertyInquiryPage, PropertyInquiryCommunication, PropertyInquiryCommunicationPage, RecordPropertyInquiryCommunicationInput, PropertyViewing, PropertyViewingScheduleInput, PropertyViewingOutcome,
...
export function createPropertyApi(tokens: AccessTokenProvider): PropertyManagementApi {
  ...
  return {
    ...
    listPropertyInquiryCommunications:(propertyId,inquiryId,cursor)=>request<PropertyInquiryCommunicationPage>(inquiryCommunicationPath(propertyId,inquiryId,cursor)),
    recordPropertyInquiryCommunication:(propertyId,inquiryId,input)=>request<PropertyInquiryCommunication>(inquiryCommunicationPath(propertyId,inquiryId),{method:"POST",body:input}),
...
function inquiryCommunicationPath(propertyId:string,inquiryId:string,cursor?:string):`/v1/${string}`{const base=`/v1/properties/${encodeURIComponent(propertyId)}/inquiries/${encodeURIComponent(inquiryId)}/communications` as `/v1/${string}`;return cursor?`${base}?limit=20&cursor=${encodeURIComponent(cursor)}`:base;}
