...
export function PropertyDetailPage() {
  ...
  const isCommercialTarget = workspace.capabilities.isCommercialTarget ?? true;
  ...
  return (
    <div className="page-stack property-page">
      <PageHeader
        actions={<>{isCommercialTarget && (leaseEligible ? <a className={buttonClassName("primary", "inline-action")} href="#property-contracts">Gérer les contrats</a> : <button className={buttonClassName("primary", "inline-action")} disabled title={leaseReason}>Gérer les contrats</button>)}<Link className={buttonClassName("secondary", "inline-action")} to="/properties">Retour aux biens</Link></>}
      ...
      <nav aria-label="Sections de la fiche" className="property-context-nav">
        ...
        {isCommercialTarget && <a aria-current={activeSection === "#property-inquiries" ? "location" : undefined} href="#property-inquiries">Demandes</a>}
        {isCommercialTarget && <a aria-current={activeSection === "#property-applications" ? "location" : undefined} href="#property-applications">Candidatures</a>}
        {isCommercialTarget && (leaseEligible ? <a aria-current={activeSection === "#property-contracts" ? "location" : undefined} href="#property-contracts">Clients et contrats</a> : <span aria-disabled="true" title={leaseReason}>Clients et contrats</span>)}
        <a aria-current={activeSection === "#property-availability" ? "location" : undefined} href="#property-availability">Disponibilité</a>
        {isCommercialTarget && <a aria-current={activeSection === "#property-pricing" ? "location" : undefined} href="#property-pricing">Tarification</a>}
        {isCommercialTarget && <a aria-current={activeSection === "#property-publication" ? "location" : undefined} href="#property-publication">Publication</a>}
        ...
        <a aria-current={activeSection === "#property-amenities" ? "location" : undefined} href="#property-amenities">Commodités et équipements</a>
      ...
      <section className="property-summary content-panel" id="property-overview" aria-labelledby="property-summary-title">
        ...
        <div className="workspace-summary-grid" aria-label="Synthèse opérationnelle">
          ...
          {isCommercialTarget && <a href="#property-publication"><strong>{workspace.publicationReadiness.ready ? "Prêt" : `${workspace.publicationReadiness.missingRequirements.length} à compléter`}</strong><span>Publication</span></a>}
          {isCommercialTarget && (leaseEligible ? <a href="#property-contracts"><strong>{workspace.contracts.activeCount} actif{workspace.contracts.activeCount > 1 ? "s" : ""}</strong><span>{workspace.contracts.totalCount} contrat{workspace.contracts.totalCount > 1 ? "s" : ""}</span></a> : <div title={leaseReason}><strong>Indisponible</strong><span>Contrats</span></div>)}
          <a href="#property-owners"><strong>{workspace.owners.length || (workspace.effectiveOwner ? 1 : 0)}</strong><span>Propriétaire{workspace.owners.length > 1 ? "s" : ""}</span></a>
      ...
      {isCommercialTarget && <div id="property-contracts">{leaseEligible ? <PropertyContractsSection
      ...
      {isCommercialTarget && <section className="content-panel" id="property-pricing" aria-labelledby="property-pricing-title">
      ...
      {isCommercialTarget && <div id="property-publication"><PropertyPublicationSection
          property={property}
      ...
      {isCommercialTarget && <PropertyInquiriesSection propertyId={property.propertyId} api={api}/>}
      {isCommercialTarget && <PropertyApplicationsSection propertyId={property.propertyId} api={api}/>}

      <div id="property-owners"><PropertyOwnershipSection propertyId={property.propertyId} api={api} initialOwners={workspace.owners} inheritedOwner={workspace.effectiveOwner} canManage={workspace.capabilities.canManageOwners} onChanged={loadWorkspace} /></div>
